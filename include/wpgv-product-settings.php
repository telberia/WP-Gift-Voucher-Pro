...
if($options->is_woocommerce_enable) {
    ...
    ?>
    <style type="text/css">
        .wpgv-tag{ padding: 8px;margin-bottom: 8px;margin-right: 4px;text-align: center;border-radius: 5px;border-width: 1px;color: #ffff;border-color: #999999;background-color: #007cba;border-style: solid; }
        .wpgv-price-remove{ margin-left: 10px; cursor: pointer; font-size: 17px; }
    </style>
    ...
    add_filter( 'woocommerce_product_add_to_cart_text', 'wpgv_archive_custom_cart_button_text' );
        function wpgv_archive_custom_cart_button_text( $text ) {
            global $product;

            if ( $product->is_type( 'gift_voucher' ) ) {
                $text = __( 'Select Amount', 'woocommerce' );
            }
            return $text;
        }
}