...
$color_preview = wpgv_hex2rgb($color_preview);
$pdf = new WPGV_PDF('P','pt',array(595,900));
$pdf->SetAutoPageBreak(0);
$pdf->AddPage();
if(get_locale() == "el"){
	$pdf->AddFont('Arial_Greek', '', 'Arial_Greek.php');
	$pdf->SetFont('Arial_Greek', '', 14);
}
...
if($formtype == 'item') {
	...
	$pdf->SetTextColor($voucher_color[0],$voucher_color[1],$voucher_color[2]);
	$pdf->SetFontSize(25);
	$pdf->MultiCell(550, 25, wpgv_em(get_the_title($product_id)), 0, 'C');
	...
	$pdf->SetTextColor($voucher_color[0],$voucher_color[1],$voucher_color[2]);
	$pdf->MultiCell(550, 12, wpgv_em(get_post_meta( $product_id, 'description', true )), 0, 'C');
}
//For
if(get_locale() != "el"){
...
if($preview) {
	...
	$pdf->SetTextColor($color_preview[0], $color_preview[1], $color_preview[2]);
	$pdf->RotatedText(75,700,wpgv_em($text_preview),45);
}